...
void result(const String& command,bool ok,const String& error) {
  ...
  JsonDocument d;d["command"]=command;d["ok"]=ok;d["status"]=ok?"accepted":"rejected";d["error"]=error;d["uptime"]=millis()/1000;

  String data;serializeJson(d,data);publish(topic("result"),data,false);

  lastResult=command+": "+(ok?String("angenommen"):error);
...
bool mqttPublishStatus() {
  ...
  d["heater_connected"]=heaterAvailable();d["driver_ready"]=heaterControlsReady();

  String data;serializeJson(d,data);bool ok=publish(topic("state"),data)>=0;

  JsonDocument h;heaterMqttState(h.to<JsonObject>());String payload;serializeJson(h,payload);
  publish(topic("heater/state"),heaterAvailable()?payload:String("{}"));
  publish(topic("heater/availability"),heaterAvailable()?"online":"offline");lastPublish=millis();return ok;
...
void mqttLoop() {
  static bool lastHeaterAvailable=false;
  static uint32_t observedRevision=0;
  static String observedResult;
  bool available=heaterAvailable();
  if(available!=lastHeaterAvailable){lastHeaterAvailable=available;forcePublish=true;}
  if(observedRevision!=heaterRevision()){
    observedRevision=heaterRevision();JsonDocument h;heaterInfo(h.to<JsonObject>());
    String current=h["result"]|"";
    if(current!=observedResult){
      observedResult=current;forcePublish=true;
      if(current.length()){
        JsonDocument event;event["status"]=current;event["source"]="heater";event["uptime"]=millis()/1000;
        String data;serializeJson(event,data);publish(topic("heater/result"),data,false);
      }
    }
  }
